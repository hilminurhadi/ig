...
const lcdm = async function(session, accountId, text){
  var result;
  ...
  if (result.length > 0) {
    ...
    return chalk`{bold.green ${printComment}:${printDM}:${printLike} » {bold.cyan ${text}}}`;
  }
      ...
      for (let i = 0; i < TargetResult.length; i++) {
        ...
        await Promise.all(TargetResult[i].map(async(akun) => {
          if (!getFollowers.includes(akun.id) && akun.params.isPrivate === false) {
            ...
            const ngeDo = await lcdm(doLogin.session, akun.id, ranText)
...
console.log(chalk`{bold.cyan
  Ξ TITLE  : CLDM [COMMENT-LIKE-DM TARGET FOLLOWER]
