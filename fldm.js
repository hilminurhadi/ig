...
},
{
  type:'input',
  name:'text',
  message:'Insert Text DM (Use [|] if more than 1):',
  validate: function(value){
    if(!value) return 'Can\'t Empty';
    return true;
  }
...
const fldm = async function(session, accountId, text){
  var result;
  ...
  if (result.length > 0) {
    ...
    return chalk`{bold.green ${printFollow}:${printDM}:${printLike} » {bold.cyan ${text}}}`;
  }
      ...
      for (let i = 0; i < TargetResult.length; i++) {
        ...
        await Promise.all(TargetResult[i].map(async(akun) => {
          if (!getFollowers.includes(akun.id) && akun.params.isPrivate === false) {
            ...
            const ngeDo = await fldm(doLogin.session, akun.id, ranText)
...
console.log(chalk`{bold.cyan
  Ξ TITLE  : FLDM [FOLLOW-LIKE-DM TARGET FOLLOWER]
