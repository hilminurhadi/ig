...
},
{
  type:'input',
  name:'text',
  message:'Insert Text DM & Comment (Use [|] if more than 1):',
  validate: function(value){
    if(!value) return 'Can\'t Empty';
    return true;
  }
...
const flcdm = async function(session, accountId, text){
  var result;
  ...
  if (result.length > 0) {
    ...
    return chalk`{bold.green ${printFollow}:${printComment}:${printDM}:${printLike} » {bold.cyan ${text}}}`;
  }
      ...
      for (let i = 0; i < TargetResult.length; i++) {
        ...
        await Promise.all(TargetResult[i].map(async(akun) => {
          if (!getFollowers.includes(akun.id) && akun.params.isPrivate === false) {
            ...
            const ngeDo = await flcdm(doLogin.session, akun.id, ranText)
...
console.log(chalk`{bold.cyan
  Ξ TITLE  : FFT [FOLLOW-LIKE-DM-COMMENT TARGET FOLLOWER]
